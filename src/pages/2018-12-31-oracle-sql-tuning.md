...
如果性能瓶颈出现在 IO 操作上，或者是 `merge` 和 `update` 等成本比较高的操作上，尝试考虑从业务上优化。这种方法能否成功只能随缘，但是如果成功效果就会很美好。目前遇到一个案例通过修改业务逻辑后，极大程度提升了效率，在之后会详细介绍。虽然方案没有普适性，但是可以参考。

- 更新的场合，结合数据分析，寻找能尽量减少更新量的方法
