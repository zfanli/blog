...
目前项目正在构建一个 ETL 系统，做数据的清洗 ✨。现在已经请求了“真实数据”，在做最后的线上环境测试。在测试的过程中，出现了几个性能上有问题的 SQL，这篇文章对解决这些性能问题的过程和方法做一个笔记。不过在进入正文之前，我想说一下问题出现的原因。

关于有问题的几条 SQL 涉及的表，在得到“真实数据”之前，我们其实是不太清楚其数据规模的，而在之前的测试中，也只是最多几百条数据的情况下做的，还体现不出来问题，但是当数据量达到千万级和亿级的时候，性能差距就很明显了。
...
这篇文章篇幅有些长，主要内容是对项目上最近解决的一些 SQL 的性能问题做一个笔记。笔记内容包括问题出现的原因，解决的过程以及结果，还有对这些问题的再现和验证。

在文章的开头，会将具体用到的所有解决方案，和尝试过的有效的解决方案做一个罗列，并且对解决问题的思路做一个粗略的总结。这部分主要对解决方案和思路做一个说明，不涉及具体案例。

接下来的一部分，会从这次遇到的问题中选择几个典型的例子 🌰 作为案例，来进一步解释说明针对这些性能问题具体运用到的解决方案以及其效果。这部分主要展示各个解决方案的效果，不会有具体代码。

最后一部分中，我们在本地测试环境用测试数据模拟出这些性能问题，并且通过不同的解决方案尝试解决问题。这部分主要重现性能问题并验证解决方案，从构筑测试环境和准备测试数据开始一步一步重现和验证几个重要的解决方案，会有大部分代码示例。

### 解决方案和思路

本次实践具体运用了的解决方法从易到难，有以下几种：
...
某些写法可能会导致无法走索引，在我们遇到的问题中，有一个是使用了 `in` 关键字导致无法走索引，最终通过改写 SQL 将其转化为 `inner join` 才使用了索引，提高了执行效率。

当优化索引还是无法解决问题的时候，就要结合更多的信息来分析了。到这一步问题可能就不是耗时过长问题了，应该会有一个报错信息，结合起来分析。如果遇到内存问题，如 PAG 写满了，或者 UNDO 空间写爆了，考虑下面的方案。

- 分割复杂 SQL 到多个简单 SQL，减少内存占有时间
- 使用 `merge` 代替 `update` 缩短更新操作时间，减少内存占有时间

目前我们遇到带报错信息的问题还只有内存问题一个。

如果以上方法都尝试过还是没有效果，这种情况一般是索引优化或者分割复杂 SQL 到头了，最终的瓶颈出在 IO 操作或者是 `merge` 和 `update` 等非常昂贵的操作上了，没地方优化了，这时就可以尝试考虑从业务上优化。

优化业务逻辑风险比较大，而且能否成功通过优化业务逻辑提升 SQL 性能还是随缘的，但是如果成功的话，效果就会很美好。目前遇到一个案例，通过修改业务逻辑后，效率从原本的 2 个小时提升到了 3 秒。这个案例在之后会详细介绍，虽然方案没有普适性，但是可以作为参考。一般优化业务逻辑要做下面的考虑。

- 更新数据的场景，结合数据分析，寻找业务逻辑的优化点

设计业务的时候往往不会太过考虑具体的 SQL 实现，就会有一个想当然的问题，就是逻辑上毫无漏洞的业务，实际上实现出来会非常“愚蠢”甚至“臃肿”。这时考虑下面的措施。

- 分析业务逻辑，将多次重复的查询固化到一张工作表中，减少反复查询

针对数据的抽取和插入，减少 IO 都是第一要考虑的要素，可以考虑下面的做法。

- 针对多步骤的业务，提取所有筛选条件到第一步，缩小数据范围

这个说法可能有些笼统，举个例子来说，假设一个业务分两步，第一步选出业务上的一个范围的数据，例如系统中存在企业和个人的数据，第一步就假设要选出所有个人的数据，而第二步要根据各种条件做关联，最终筛选一部分个人数据将某个 Flag 更新掉，更新的条件是当其为 `1` 则更新为 `0`，反之为 `0` 就更新成 `1`。

按照业务设计的逻辑来实现 SQL，那么结果就是先抽出所有个人数据，然后在这个数据量上进行第二步的操作。但是本着减少 IO 就是提高效率的思路来看，将第二步的更新条件提取到第一个数据抽取的过程是最优化的，因为更新条件中限定仅对这个 Flag 值为 `1` 或者 `0` 的时候才进行操作，这其实是一个筛选条件，如果放在第一步，就可能减少后续的数据访问量，提高执行效率。

具体实践的解决方案就是以上这些，除此之外还会稍微记录一下下面这些解决方案，这些方案有些是研讨中还没有运用到实际问题上去的，不过都是经过验证有效的方案。

- 使用 `partition` 表分区提高效率
- 使用 `/*+ APPEND */` 和 `nologging` 减少日志提高写入效率
